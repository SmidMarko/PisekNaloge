...
var makeTurtle = function(coords) {
	...
	//customBlocks
	this.drawSunflower = function(){
		paint = this.paint;
		angle = this.direction*180/Math.PI;
		color = this.drawingContext.strokeStyle;
		
		this.stop_painting();
		this.set_colour("#ffcc00");
		this.turn(-angle);
...
var getContext = function(display, infos) {
	var localLanguageStrings = {
		...
		sl: {
			...
			messages: {						//Besedila sporočil
				paintingWrong: "Pišek ni pravilno zasadil sončnic.",
				paintingCorrect: "Čestitamo! Pišek je pravilno zasadil sončnice.",
				paintingFree: "Pišek je narisala vzorec, ki si ga sprogramiral. Če ga želiš shraniti, posnemi sliko zaslona.",
			},
		},
		none: {
			comment: {
	...
	context.turtle.drawSunflower = function(callback) {
		callOnAllTurtles(function(turtle) {
	...
	context.turtle.sunflower = function(callback) {
		context.turtle.drawSunflower(callback);
	}
